...
(test webhook-post-sends-json-payload-and-content-type
  (let ((old-http amoebum::*webhook-http-request-function*)
        (captured-method nil)
        (captured-headers nil)
        (captured-content nil))
    (unwind-protect
        (progn
          (setf amoebum::*webhook-http-request-function*
          ...
          (multiple-value-bind (ok detail)
              (amoebum:send-webhook-notification
               (amoebum:make-webhook-config
            ...
            (is-true ok)
            (is (null detail))))
      (setf amoebum::*webhook-http-request-function* old-http))
...
(test webhook-retries-on-5xx-with-exponential-backoff
  (let ((old-http amoebum::*webhook-http-request-function*)
        (old-sleep amoebum::*webhook-sleep-function*)
        (attempts 0)
        (sleep-calls '()))
    (unwind-protect
        (progn
                ...
                (lambda (_url &key &allow-other-keys)
                  ...
                  (if (< attempts 3)
                      ...
                      (list :status 200
                            ...
                            :effective-url "https://example.test/webhook"
                            :content-type "application/json"))))
          (setf amoebum::*webhook-sleep-function*
                (lambda (seconds)
          ...
          (multiple-value-bind (ok detail)
            ...
            (is (null detail))))
      (setf amoebum::*webhook-http-request-function* old-http
            amoebum::*webhook-sleep-function* old-sleep))
    (is (= 3 attempts))
