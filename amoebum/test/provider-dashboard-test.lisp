...
(test providers-slash-command-toggles-dashboard-visibility
  (let* ((chat-state (amoebum:make-chat-ui-state :stream-runner nil))
         (handle-slash-input
           (symbol-function
    ...
    (is-true (amoebum:chat-ui-state-provider-dashboard-visible-p chat-state))
    (is-true (funcall handle-slash-input chat-state "/providers off"))
    (is-false (amoebum:chat-ui-state-provider-dashboard-visible-p chat-state))
