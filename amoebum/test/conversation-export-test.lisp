...
(test export-api-functions-exist
  ...
  (is (fboundp 'amoebum:export-conversation-markdown))
  (is (fboundp 'amoebum:export-conversation-json))
  (is (fboundp 'amoebum:export-conversation)))
...
(test export-markdown-returns-string
  ...
  (let* ((conv (%make-test-conversation))
         (md (amoebum:export-conversation-markdown conv)))
    (is (stringp md))
    (is (plusp (length md)))))
...
(test export-json-returns-string
  ...
  (let* ((conv (%make-test-conversation))
         (json (amoebum:export-conversation-json conv)))
    (is (stringp json))
...
(test export-json-valid-structure
  ...
  (let* ((conv (%make-test-conversation))
         (json (string-trim '(#\Space #\Newline #\Return #\Tab)
                            (amoebum:export-conversation-json conv))))
    (is (char= #\{ (char json 0)))
  ...
  (let ((old-dir amoebum:*conversation-export-directory*)
    ...
    (unwind-protect
         (progn
           ...
           (let* ((conv (%make-test-conversation))
                  (path (amoebum:export-conversation conv
