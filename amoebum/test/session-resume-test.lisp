...
(defun %i338-build-saved-session (project-root session-id)
  (let* ((tool-call-id (format nil "call-~A" session-id))
         (conversation (amoebum.sessions:make-conversation-state
...
(test cli-resume-by-id-is-deterministic
  (let* ((tmp-root (%make-temp-directory "amoebum-i338-cli"))
         (old-project-root (amoebum.config:config-project-root (amoebum.config:current-config))))
    (unwind-protect
         (progn
           ...
           (let ((resolved (amoebum::%resolve-cli-conversation :resume "resume-alpha")))
             (is (string= "resume-alpha"
                          (amoebum.sessions:conversation-state-session-id resolved)))
             (is (eq :tool-executing
                     (amoebum.sessions:conversation-state-state resolved)))
             (is (= 3 (length (amoebum.sessions:conversation-state-entries resolved))))
             (let ((third (third (amoebum.sessions:conversation-state-messages resolved))))
         ...
         (progn
           ...
           (multiple-value-bind (handled result)
             ...
             (is-true (search "Resumed session resume-slash"
                              ...
                              :test #'char-equal)))
           (let* ((conversation (amoebum.ui:chat-ui-state-conversation chat-state))
                  (messages (amoebum.ui:chat-ui-state-messages chat-state))
                  (third (third messages)))
             (is (string= "resume-slash"
                          (amoebum.sessions:conversation-state-session-id conversation)))
             (is (eq :tool-executing
                     (amoebum.sessions:conversation-state-state conversation)))
    ...
    (unwind-protect
         (progn
           ...
           (let* ((conversation (%i338-build-saved-session tmp-root "checkpoint-resume"))
                  (checkpoint (amoebum.sessions:checkpoint-session :conversation conversation
             ...
             (let* ((conversation (amoebum.ui:chat-ui-state-conversation chat-state))
                    (messages (amoebum.ui:chat-ui-state-messages chat-state))
                    (status-state (amoebum.ui:chat-ui-state-status-bar-state chat-state)))
               (is (string= "checkpoint-resume"
                            (amoebum.sessions:conversation-state-session-id conversation)))
               (is (= 3 (length messages)))
               (is-true (plusp (amoebum.ui:chat-ui-state-context-used-tokens chat-state)))
               (is (= (amoebum.ui:chat-ui-state-context-used-tokens chat-state)
                      (amoebum.ui:status-bar-state-context-used-tokens status-state))
