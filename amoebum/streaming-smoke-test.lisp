  ...
  (let* ((amoebum-pkg (or (find-package "AMOEBUM")
    ...
    (labels
           ...
           (let ((compact-segment-p-fn (funcall fn-in "COMPACT-SEGMENT-P" amoebum-pkg)))
             (if (funcall compact-segment-p-fn segment)
                 (let* ((lookup-style-fn (funcall fn-in "LOOKUP-STYLE" amoebum-pkg))
                        (compact-style-id-fn (funcall fn-in "COMPACT-SEGMENT-STYLE-ID" amoebum-pkg))
                        ...
                        (style-entry-underlinep-fn (funcall fn-in "STYLE-ENTRY-UNDERLINEP" amoebum-pkg))
      ...
      (let* ((bold-lines
        ...
        (assert-true (and bold-segment (string= (segment-text bold-segment) "bold"))
                     "Expected partial bold markdown text to strip marker, got ~S."
                     bold-segment)
        (assert-true (segment-field bold-segment :boldp)
                     "Expected partial bold markdown to keep :BOLDP style.")
        (assert-true (and italic-segment (string= (segment-text italic-segment) "ital"))
                     "Expected partial italic markdown text to strip marker, got ~S."
                     italic-segment)
        (assert-true (segment-field italic-segment :italicp)
                     "Expected partial italic markdown to keep :ITALICP style.")
        (assert-true (and code-segment (string= (segment-text code-segment) "code"))
                     "Expected partial code markdown text to strip marker, got ~S."
                     code-segment)
        (assert-true (eq (segment-field code-segment :role) :assistant-inline-code)
                     "Expected partial code markdown to use :ASSISTANT-INLINE-CODE role.")
        (assert-true (and heading-segment
                          (string= (segment-text heading-segment) "Heading"))
                     "Expected markdown heading text to strip prefix, got ~S."
                     heading-segment)
        (assert-true (eq (segment-field heading-segment :role) :assistant-heading)
