...
;;; Streaming markdown rendering extracted from ui/streaming.lisp as the
;;; fallback NXT-383 ownership cluster. Token-stream state ownership remains in
;;; src/ui/streaming/token-stream.lisp and provider orchestration remains in
;;; src/ui/streaming/provider-runtime.lisp.
         ...
         (word-start-p (pos)
           (or (zerop pos)
               (let ((ch (char line (1- pos))))
                 (not (or (alphanumericp ch) (char= ch #\_) (char= ch #\-)))))))
      (declare (ignore word-boundary-p))
...
(defstruct (streaming-markdown-renderer
  ...
  (fenced-language nil)
  ;; Tail pointers keep append O(1) instead of traversing the whole transcript.
