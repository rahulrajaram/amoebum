    ...
    (labels ((assert-true (condition format-string &rest format-args)
      ...
      (let ((state (make-chat-state)))
        ...
        (let* ((buffer (funcall render-chat-ui-buffer-fn
               ...
               (rows (buffer-lines buffer))
               (system-fg (prefix-fg-string buffer ">")))
          (assert-true (rows-contain-p rows "Show me the diff.")
                       "Expected rendered history to include user message text.")
          (assert-true (rows-contain-p rows "I found 3 changed files.")
                       "Expected rendered history to include assistant message text.")
          (assert-true (rows-contain-p rows "> System primed.")
                       "Expected rendered history to include system role prefix/text.")
          (assert-true (not (rows-contain-p rows "YOU>"))
                       "Expected user role prefix to be omitted from rendered history.")
          (assert-true (not (rows-contain-p rows "ASSISTANT>"))
                       "Expected assistant role prefix to be omitted from rendered history.")
          (assert-true system-fg
                       "Expected to locate the styled system role prefix in rendered buffer.")))
