...
(defun %server-send-response (stream status reason body headers)
  (let ((payload (or body "")))
    (format stream "HTTP/1.1 ~A ~A~C~C" status reason #\Return #\Linefeed)
...
(defun stop-mock-provider-server (server)
  (let ((stop-flag (mock-provider-server-stop-flag server))
        ...
        (port (mock-provider-server-port server)))
    (when stop-flag
      (setf (car stop-flag) t))
    ;; Nudge blocking ACCEPT so the thread can observe STOP-FLAG and exit.
    (ignore-errors
      (let ((poke (usocket:socket-connect "127.0.0.1" port)))
        (when poke
          (usocket:socket-close poke))))
    (ignore-errors (usocket:socket-close socket))
    (when thread
      (ignore-errors (sb-thread:join-thread thread :timeout 1.0))
      (when (and (fboundp 'sb-thread:thread-alive-p)
    ...
    (let* ((provider (pseudopod:make-anthropic-provider
      ...
      (let ((result (pseudopod:send-streaming-completion
        ...
        (let ((tool-calls (%tool-calls-list (gethash "tool_calls" result))))
          (is (vectorp (gethash "tool_calls" result)))
  ...
  (with-mock-provider-server (server)
    (let* ((provider (pseudopod:make-openai-compatible-provider
      ...
      (handler-case
        ...
        (dexador.error:http-request-too-many-requests (condition)
          (declare (ignore condition))
          (setf status-code 429))
        (error (condition)
          (when (search "429" (princ-to-string condition))
            (setf status-code 429))))
      (is (= 429 status-code))
